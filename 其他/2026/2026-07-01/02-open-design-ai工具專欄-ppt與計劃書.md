...
3. **把研究筆記轉成可演示內容**  
   對這個 `industry-ai-notes` 很有啟發：Markdown 筆記可以先沉澱邏輯，再用 Open Design / HTML deck 模板轉成演講或簡報。
...
1. 用同一份 `industry-ai-notes` 筆記，生成 HTML deck、PDF、PPTX，檢查三者版面保真度。
